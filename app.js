...
var times = [];
var output = [ results, places, times ];
      ...
      for (var i=0;i<watchSymbols.length;i++){
        ...
        for (var j=0;j<this_param.length;j++){
          if (lowtweet.search(this_param[j]) >= 0){
            ...
            var team1 = gamesArr.team1;
            var team2 = gamesArr.team2;
            for (var j=0;j<results.length;j++){ 
              if (results[2][j] == team1){
                gamesArr.team1_colors = results[3][j];
                gamesArr.team1_mascot = results[5][j];
                gamesArr.team1_conf = results[4][j];
                gamesArr.team1_count = results[1][j];
              } if (results[2][j] == team2){
                gamesArr.team2_colors = results[3][j];
                gamesArr.team2_mascot = results[5][j];
                gamesArr.team2_conf = results[4][j];
                gamesArr.team2_count = results[1][j];
              }
            } 
